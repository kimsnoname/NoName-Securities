...
const NoticeForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEdit && noticeId) {
      ...
      axios.get('http://43.202.240.147:8080/api/notices')
        .then(response => {
          setNotices(response.data);
        })
        .catch(error => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        const response = await axios.put(`http://43.202.240.147:8080/api/notices/${noticeId}`, newNotice);
        setNotices(notices.map(n => (n.noticeId === newNotice.noticeId ? response.data : n)));
      } else {
        const response = await axios.post('http://43.202.240.147:8080/api/notices', newNotice);
