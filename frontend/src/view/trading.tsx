...
const axiosInstance = axios.create({
	baseURL: 'http://www.nonamestock.com:5002',
...
const Trading: React.FC = () => {
	...
	useEffect(() => {
		const fetchStockData = async (code: string) => {
			try {
				const response = await axiosInstance.get<StockData>(`/stock/${code}`);
	...
	useEffect(() => {
		const fetchUserQuantity = async () => {
			...
			try {
				...
				const response = await axiosInstance.get<{ quantity: number }>(`http://43.202.240.147:8080/api/user/stock/${stockCode}`, {
					headers: {
	...
	useEffect(() => {
		const fetchImage = async () => {
			try {
				...
				// 이미지 URL 생성
				const response = await axiosInstance.get(`http://43.202.240.147:8080/api/user/image?imageUrl=${imageUrl}`);
