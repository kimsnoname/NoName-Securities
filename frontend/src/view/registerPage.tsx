...
const Register: React.FC = () => {
  ...
  const checkDuplicateUserName = async () => {
    setErrorMessage('');
    try {
      const response = await axios.post('http://43.202.240.147:8080/api/user/check-username', { userName });
  ...
  const handleEmailVerification = async () => {
    setErrorMessage('');
    try {
      const response = await axios.post('http://43.202.240.147:8080/api/user/check-email', { email });
      if (response.status === 200) {
  ...
  const handleSubmit = async() => {
    ...
    try {
      const response = await axios.post('http://43.202.240.147:8080/api/user/register', data, 
        {
          headers: {
