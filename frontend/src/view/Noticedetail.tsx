...
const NoticeDetail: React.FC = () => {
    ...
    const fetchNoticeAndReplies = async () => {
      try {
        const [noticeResponse, repliesResponse] = await Promise.all([
          axios.get(`http://43.202.240.147:8080/api/notices/${noticeId}`),
          axios.get(`http://43.202.240.147:8080/api/replies/notice/${noticeId}`)
        ]);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://43.202.240.147:8080/api/notices/${noticeId}`);
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      await axios.post('http://43.202.240.147:8080/api/replies', newReplyData);
  ...
  const handleReplyDelete = async (replyId: number) => {
    try {
      await axios.delete(`http://43.202.240.147:8080/api/replies/${replyId}`);
