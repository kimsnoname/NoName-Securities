...
const CommunityForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBoardPosts = async () => {
      try {
        const response = await axios.get(`http://43.202.240.147:8080/api/posts/${boardId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        const response = await axios.put(`http://43.202.240.147:8080/api/posts/${boardIdInput}/${id}`, newPost);
        setPosts(posts.map(p => (p.id === newPost.id && p.boardId === newPost.boardId ? response.data : p)));
      } else {
        const response = await axios.post('http://43.202.240.147:8080/api/posts', newPost);
