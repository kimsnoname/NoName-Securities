...
const CommunityDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://43.202.240.147:8080/api/posts/${boardId}/${id}`);
        setPost(response.data);
        const commentsResponse = await axios.get(`http://43.202.240.147:8080/api/replies/${boardId}/${id}`);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://43.202.240.147:8080/api/posts/${boardId}/${id}`);
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post(`http://43.202.240.147:8080/api/reply/add`, {
        ...newCommentData,
        boardId,
        postId: id,
      });
  ...
  const handleCommentDelete = async (replyId: number) => {
    try {
        const response = await axios.delete(`http://43.202.240.147:8080/api/delete/reply/${boardId}/${id}/${replyId}`);
