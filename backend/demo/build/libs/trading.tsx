...
axios.defaults.baseURL = 'http://www.nonamestock.com:5002';
...
const Trading: React.FC = () => {
	...
	useEffect(() => {
		const fetchStockData = async (code: string) => {
			try {
				const response = await axios.get<StockData>(`http://www.nonamestock.com:5002/stock/${code}`);
	...
	useEffect(() => {
		const fetchUserQuantity = async () => {
			...
			try {
				...
				const response = await axios.get<{ quantity: number }>(`http://43.202.240.147:8080/api/user/stock/${stockCode}`, {
					headers: {
	...
	const handlePasswordSubmit = async () => {
		...
		try {
			const response = await axios.post(
				'http://43.202.240.147:8080/api/transfer/verifypassword',
				{
					userId,
					accountPassword
				},
			...
			if (response.data.success) {
				...
				try {
					const response = await axios.post(
						`http://43.202.240.147:8080/api/transaction/${tradeType}`, data,
						{
							headers: {
				...
				setAccountPassword("");
				if (attempts + 1 >= 5) {
					message.error("비밀번호를 5회 이상 틀렸습니다. 거래가 불가능합니다.");
					handleCancel();
				} else {
	...
	const handleSearch = () => {
		...
		const fetchUserQuantity = async () => {
			...
			try {
				const token = localStorage.getItem('token'); // 로컬 스토리지에서 JWT 토큰 가져오기
				const response = await axios.get<{ quantity: number }>(`http://192.168.0.100:8080/api/user/stock/${stockCode}`, {
					headers: {
						Authorization: `Bearer ${token}`
					}
				});
										...
										<Col span={11}>
											<div className="other-info">
												<Card size="small" style={{ width: '90%' }}>
													<Card.Grid style={gridStyle}>저가: {stockData.low_price}원</Card.Grid>
													<Card.Grid style={gridStyle}>예상체결가: {stockData.expected_closing_price}원</Card.Grid>
													<Card.Grid style={gridStyle}>고가: {stockData.high_price}원</Card.Grid>
													<Card.Grid style={gridStyle}>거래량: {stockData.volume}주</Card.Grid>
												</Card>
											</div>
										</Col>
									) : (
